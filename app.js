(() => {
  ...
  function setMarketModel(key, model) {
  ...
  async function loadMarketService() {
    try {
      ...
      const btcPrice = num(btc.usd), btcChange = num(btc.usd24hChange);
      if (Number.isFinite(btcPrice)) {
        latestBtcPrice = btcPrice; currentBtcChange = btcChange;
        $("ticker-btc").textContent = fmtUsd(btcPrice, 0);
        $("ticker-btc-change").textContent = signed(btcChange);
        tone($("ticker-btc-change"), btcChange);
      }
      ...
      if (Number.isFinite(marketCap)) $("global-mcap").textContent = fmtUsd(marketCap);
      if (Number.isFinite(volume)) $("global-volume").textContent = fmtUsd(volume);
      if (Number.isFinite(btcDominance)) { $("global-btc-dom").textContent = signed(btcDominance, "%", 1).replace("+", ""); }
  ...
  async function loadSentimentService() {
    try {
      ...
      $("gauge-value").textContent = latest.value;
  ...
  async function loadOnchainService() {
    try {
      ...
      if (Number.isFinite(fast)) $("fee-fast").textContent = feeText(fast);
