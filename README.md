...
Pages 页面约每 5 分钟检查新快照；手动“刷新”及返回页面时也会按条件检查。通过 `manifest.json` 与 `static.json` 原位更新静态数据，并重取动态快照，不刷新整页或主动跳回顶部；选中的期限仍存在时保留选择。代码/样式改版仍需重新加载页面才能生效。
...
- `app.js`：数据读取、缓存、原位刷新、来源状态与基础图表；向体验层提供运行时数据。
